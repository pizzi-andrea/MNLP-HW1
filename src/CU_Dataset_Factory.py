...
class CU_Dataset_Factory:
    def __init__(self, target_feature:str, batch_size:int=1, features_enable:list[str]=['label']) -> None:
    ...
    def __produce(self, dataset:pd.DataFrame, encode:bool) -> pd.DataFrame:
        ...
        for feature in tqdm(self.features_enable, desc='copy dataset'):
            if feature in dataset.columns.tolist(): 
                if encode and (dataset[feature].dtype == pd.StringDtype() or dataset[feature].dtype == object):
    ...
    def produce(self, out_dir:path.PosixPath, encoding:bool=False, train:bool=True) -> pd.DataFrame:
        ...
        if train: # some cases need train set
            ...
            prc_validation.to_csv(out_dir.joinpath('validation.tsv'), sep='\t', mode='w')
